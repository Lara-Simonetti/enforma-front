...
@Component({
  selector: 'app-persona-terminar',
  templateUrl: './persona-terminar.component.html',
  styleUrls: ['./persona-terminar.component.css']
})
export class PersonaTerminarComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.personaService.darPersona(personaId).subscribe((persona) => {
      ...
      this.personaForm = this.formBuilder.group({
        ...
        entrenando: false,
        razon: ["", [Validators.required, Validators.minLength(2)]],
        terminado: ["", [Validators.required, Validators.minLength(10)]],
      });
    });

  }

  terminarPersona(persona: Persona): void {
